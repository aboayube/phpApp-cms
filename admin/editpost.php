...
$msg= '<div class="alert alert-danger">يجيب انت تدخل  المقال</div>';
        ...
        $allow=array('png','gif','jpeg','jpg');
         ...
         $stmt= $con->prepare("UPDATE `posts` SET `title`=?,`post`=?,`cat`=?,`img`=?,`auther`=?,`status`=?,`regdata`=now() WHERE id=?");
$stmt->execute(array($title,$post,$cat,$img_db,$_SESSION['user'],$status,$_POST['id']));            
        echo '<div class="alert alert-success">تم تحديث المقال</div>';
         ...
         $stmt= $con->prepare("UPDATE `posts` SET `title`=?,`post`=?,`cat`=?,`img`=?,`auther`=?,`status`=?,`regdata`=now() WHERE id=?");
$stmt->execute(array($title,$post,$cat,$img,$_SESSION['user'],$status,$_POST['id']));            
        echo '<div class="alert alert-success">تم تحديث المقال</div>';
                            //طريقة للانتقال الي صفحة الرئيسية
                            echo '<meta http-equiv="refresh" content="3; \'index.php\' ">';
...
?>
  ...
  <div class="panel-heading"><b>تعديل مقال <?php echo $post['title']?></b></div>
...
<form class="form-horizontal" action="" method="post" enctype="multipart/form-data">
  ...
  <div class="form-group">
    ...
    <div class="col-sm-5">
    <textarea rows="8" name="post" class="form-control"  ><?php echo $post['post'];?></textarea>
    </div>
  </div>
      <div class="form-group">
    ...
    <select name="cat" class="form-control" >
        ...
        foreach($cats as $cat){
            ...
            if($post['cat']==$cat['id']){echo 'selected';}
         echo '>'.$cat['cat'].'</option>';
        }
        ?>
    ...
    <select name="status" class="form-control" >
        <option value="derft" <?php if($post['status']=='derft'){echo 'selected';}?>>تعطيل </option>
        <option value="publish" <?php if($post['status']=='publish'){echo 'selected';}?>>نشر</option>
